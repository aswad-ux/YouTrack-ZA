      ...
      <div className="container mx-auto px-4">
        ...
        {variant === 'personal' ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 auto-rows-auto lg:auto-rows-[1fr]">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm flex flex-col hover:-translate-y-1 transition-transform">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="lg:col-span-1 lg:row-span-2 bg-gradient-to-br from-[#0F172A] to-[#1C5795] text-white border border-[#1e3a5f] rounded-3xl p-8 shadow-2xl flex flex-col relative overflow-hidden ring-1 ring-black/5 hover:-translate-y-1 transition-transform">
              ...
              <ul className="space-y-5 mb-8 flex-grow relative z-10">
                {pricingPackages[3].features.map((f, i) => (
                  <li key={i} className="flex items-start gap-3"><Check className="w-5 h-5 shrink-0 text-amber-400" /><span className="font-light text-sm">{f}</span></li>
                ))}
              </ul>
              
              <Link href="?package=Overwatch#contact" className="mt-auto w-full py-4 text-center font-bold tracking-wide rounded-xl bg-amber-500 text-slate-900 hover:bg-amber-400 transition-colors shadow-lg relative z-10">Deploy Overwatch</Link>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="lg:col-span-2 bg-slate-100/80 border border-slate-200 rounded-3xl p-8 shadow-sm flex flex-col sm:flex-row gap-8 hover:-translate-y-1 transition-transform">
              <div className="flex-1 flex flex-col">
                ...
                <p className="text-text-secondary text-sm font-light mb-6">Live Video Telematics</p>
                <div className="mt-auto pt-6">
                  <span className="text-4xl font-extrabold tracking-tighter text-text-primary">R250</span>
                  <span className="text-text-secondary font-light">/mo</span>
                </div>
              </div>
              <div className="flex-1 flex flex-col border-t sm:border-t-0 sm:border-l border-slate-200 pt-6 sm:pt-0 sm:pl-8">
